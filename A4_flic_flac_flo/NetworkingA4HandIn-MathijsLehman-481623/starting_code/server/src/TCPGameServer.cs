...
namespace server
{
    ...
    class TCPGameServer
    {
        ...
        public GameRoom GetGameRoom(int _gameInstance)
        {
            foreach (GameRoom gameRoom in _gameRooms)
            {
                if (gameRoom.instance == _gameInstance) { return gameRoom; }
            }
            return null;
        }
        ...
        public GameRoom CreateGameRoom()
        {
            ...
            newGameRoom.instance = _gameInstance;
            _gameRooms.Add(newGameRoom);
            return newGameRoom;
        }

        public void AddPlayerInfo(TcpMessageChannel pClient, PlayerInfo pInfo)
        {
            _playerInfo[pClient] = pInfo;
        }

        public void removeGameRoom(GameRoom gameRoom)
        {
            _gameRooms.Remove(gameRoom);
            Log.LogInfo("Deleted gameRoom number: " + gameRoom.instance, this);
        }
