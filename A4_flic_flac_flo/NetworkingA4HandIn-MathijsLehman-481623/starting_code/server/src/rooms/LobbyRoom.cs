...
namespace server
{
	...
	class LobbyRoom : SimpleRoom
	{
		...
		protected override void addMember(TcpMessageChannel pMember)
		{
			...
			simpleMessage.message = "Client " + _server.GetPlayerInfo(pMember).name + " has joined the lobby!";
		...
		protected override void handleNetworkMessage(ASerializable pMessage, TcpMessageChannel pSender)
		{
			...
			else if (pMessage is ChatMessage) handleLobbyMessage(pMessage as ChatMessage, pSender);
		}
		...
		private void handleReadyNotification(ChangeReadyStatusRequest pReadyNotification, TcpMessageChannel pSender)
		{
			...
			//do we have enough people for a game and is there no game running yet? Don't have to check anymore if game is already running since we'll be creating a new game room for every 2 players ready
		...
		private void startAGame(TcpMessageChannel player1, TcpMessageChannel player2)
		{
			_server.CreateGameRoom().StartGame(player1, player2);
			ChatMessage gameStartedMessage = new ChatMessage();
			gameStartedMessage.message = _server.GetPlayerInfo(player1).name + " and " + _server.GetPlayerInfo(player2).name + " have started a game against each other";

            sendToAll(gameStartedMessage);
		}

		private void handleLobbyMessage(ChatMessage pMessage, TcpMessageChannel pSender)
		{
			string senderName = _server.GetPlayerInfo(pSender).name;
			ChatMessage appendedMessage = new ChatMessage();
			appendedMessage.message = senderName + ": " + pMessage.message;
			sendToAll(appendedMessage);
		}
